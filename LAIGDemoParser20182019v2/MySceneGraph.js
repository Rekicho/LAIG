...
class MySceneGraph {
		...
		for (var i = 0; i < children.length; i++) {
			switch (children[i].nodeName) {
				...
				case "ortho":
					...
					break;
				default:
					return "unkown tag " + children[i].nodeName + " in views";
			}
		}
	...
	parseTransformationBlock(transformarionblockNode) {
		...
		for (var i = 0; i < transformarionblockNode.length; i++) {
			...
			switch (transformarionblockNode[i].nodeName) {
				...
				default:
					return "unknown transformation " + transformarionblockNode[i].nodeName;
			}
		}
	...
	parseAnimations(animationsNode) {
		...
		for (var i = 0; i < children.length; i++) {
			switch (children[i].nodeName) {
				case "linear":
					if ((error = this.parseLinearAnimation(children[i])) != null)
						return error;
					break;
				case "circular":
					if ((error = this.parseCircularAnimation(children[i])) != null)
						return error;
					break;
				default:
					this.onXMLMinorError("unknown tag <" + children[i].nodeName + ">");
		...
		for (var i = 0; i < children.length; i++) {
			...
			switch (nodeName) {
				...
				case "torus":
					...
					break;
				case "plane":
					if ((error = this.parsePlane(grandChildren[0], primitiveId)) != null)
						return error;
					break;
				case "patch":
					if ((error = this.parsePatch(grandChildren[0], primitiveId)) != null)
						return error;
					break;
				case "vehicle":
					...
					break;
				case "cylinder2":
					if ((error = this.parseCylinder2(grandChildren[0], primitiveId)) != null)
						return error;
					break;
				case "terrain":
					if ((error = this.parseTerrain(grandChildren[0], primitiveId)) != null)
						return error;
					break;
				case "water":
					if ((error = this.parseWater(grandChildren[0], primitiveId)) != null)
						return error;
					break;
		...
		for (var i = 0; i < children.length; i++) {
			...
			if (animationsIndex != -1) {
				...
				for (var j = 0; j < greatChildren.length; j++) {
					...
					//Need to "clone" animation so it can be used by different components at diferent times
					var animationInfo = [];
					this.animations[animationId].getAnimationInfo(animationInfo);

					if(animationInfo["type"] == "linear")
						animations.push(new MyLinearAnimation(this.scene,animationInfo["time"],animationInfo["controlPoints"]));

					if(animationInfo["type"] == "circular")
						animations.push(new MyCircularAnimation(this.scene,animationInfo["time"],animationInfo["center"],animationInfo["radius"],animationInfo["startang"],animationInfo["rotang"]));
				}
			}
			...
			for (var j = 0; j < greatChildren.length; j++) {
				switch (greatChildren[j].nodeName) {
					...
					default:
						this.onXMLMinorError("unknown tag <" + greatChildren[j].nodeName + "> in children for component id=" + componentId);
						continue;
				}
			}
